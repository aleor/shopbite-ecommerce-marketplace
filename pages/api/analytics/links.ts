...
import { getLinksClicked, getWebsiteLinksClicked } from '../../../libs/api/reports';
...
export default async function handler(req, res) {
  ...
  const [linksAnalyticsReport, websiteLinksClickedReport] = await Promise.all([
    getLinksClicked(shopId, startDate, endDate),
    getWebsiteLinksClicked(shopId, startDate, endDate),
  ]);

  res.json({
    linksAnalyticsReport,
    websiteLinksClickedReport,
  });
}
